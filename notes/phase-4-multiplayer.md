...
- **Contract consolidation:** instead of separate `turn:rolled` / `turn:selected` / `turn:farkled` server events (plan §4 sketch), the server broadcasts the engine's `GameLogEvent[]` stream (`game:events`) plus full `room:state` snapshots. Strictly more information, one source of truth (the same events replay/analytics already consume), and no per-event schema drift. `turn:timedOut`, `game:started`, `game:ended`, chat and presence events remain as named events.
- **`/auth/guest` reuses a valid session** instead of minting a new identity — found by the reconnect e2e: a reloaded client re-POSTs /auth/guest, and without reuse it lost its held seat. This is decision 16's "re-presents its cookie" made concrete.
- **e2e turn-driver races:** Playwright's visibility-check-then-click pattern races the server re-render; fixed with a 5s actionTimeout + retry-tolerant driver loop rather than slowing the server down.
- Banners resolve display names via the room snapshot (raw guest ids leaked into the UI otherwise).
